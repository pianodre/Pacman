...
public class GameEngineImpl implements GameEngine {
    ...
    private void initializeGhosts() {
        for (Renderable renderable : currentLevel.getRenderables()) {
            if (renderable instanceof Ghost) {
                // Set initial ghost mode or speed if needed
                Ghost ghost = (Ghost) renderable;
                ghost.setGhostMode(GhostMode.SCATTER); // or any default mode
            }
        }
    }
    ...
    public void tick() {
        ...
        currentLevel.tick();
        updateGhosts(); // Update ghost behaviors on each tick
    }
    ...
    private void updateGhosts() {
        for (Renderable renderable : currentLevel.getRenderables()) {
            if (renderable instanceof Ghost) {
                ...
                ghost.updateBlinky(currentLevel.getBlinkyPosition()); // Pass Blinky's position to ghost
            }
        }
    }
